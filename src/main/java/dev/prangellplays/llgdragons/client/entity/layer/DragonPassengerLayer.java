...
public class DragonPassengerLayer<T extends DragonEntity> extends GeoRenderLayer<T> {
    ...
    @Override
    public void renderForBone(MatrixStack matrixStackIn, T animatable, GeoBone bone, RenderLayer renderType, VertexConsumerProvider bufferSource, VertexConsumer buffer, float partialTick, int packedLight, int packedOverlay) {
        if (!bone.getName().equals(passengerBone)) return;

        Entity passenger = animatable.getOwner().getPassengerList().size() > passengerNumber ? animatable.getOwner().getPassengerList().get(passengerNumber) : null;
        if (passenger != null) {
            matrixStackIn.push();
            ...
            float yaw = MathHelper.lerpAngleDegrees(partialTick, animatable.getOwner().prevBodyYaw, animatable.getOwner().bodyYaw);
